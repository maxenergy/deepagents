...
async function runTest() {
    ...
    try {
        ...
        try {
            ...
            console.log('初始化 WorkflowCommandHandler');
            handler.initialize();
            
            // 测试创建工作流命令
            console.log('测试 createWorkflow 命令');
            await vscode.commands.executeCommand('deepagents.createWorkflow');
            
            // 获取所有工作流
            const workflows = await mockWorkflowManager.getAllWorkflows();
            ...
            if (workflows.length > 0) {
                console.log('测试 executeWorkflow 命令');
                await vscode.commands.executeCommand('deepagents.executeWorkflow', workflows[0].id);
            }
        } catch (importError) {
